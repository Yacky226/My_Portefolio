...
export function About() {
  ...
  const skills: Array<{ name: string; level: number }> = useMemo(() => {
  ...
  const rawAchievements: any = t("about.achievements");
  const achievements = useMemo(() => {
    return achievementsMeta.map((meta, idx) => {
      const item = Array.isArray(rawAchievements) ? rawAchievements[idx] : null;
      const title = item?.title ?? t(`about.achievements.${idx}.title`) ?? "";
      const subtitle =
        item?.subtitle ?? t(`about.achievements.${idx}.subtitle`) ?? "";
  ...
  const rawTimeline: any = t("about.timeline");
  const timeline: Array<any> = useMemo(() => {
  ...
  return (
    <section
      ref={sectionRef}
      id="about"
      className="py-20 bg-surface/30 section-reveal"
      style={{ backgroundColor: "rgba(11, 18, 32, 0.3)" }}
    >
